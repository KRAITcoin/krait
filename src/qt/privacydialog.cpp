                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zKRAIT ought to be enough for anybody." - Bill Gates, 2017
    ui->zKRAITpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzKRAITSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzKRAIT->setEnabled(false);
        ui->pushButtonMintzKRAIT->setToolTip(tr("zKRAIT is currently disabled due to maintenance."));

        ui->pushButtonSpendzKRAIT->setEnabled(false);
        ui->pushButtonSpendzKRAIT->setToolTip(tr("zKRAIT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zKRAITpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzKRAIT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKRAIT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zKRAIT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zKRAIT in ") + 
...
void PrivacyDialog::on_pushButtonSpendzKRAIT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKRAIT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zKRAIT
        sendzKRAIT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zKRAIT
    sendzKRAIT();
}

void PrivacyDialog::on_pushButtonZKRAITControl_clicked()
{
    ZKRAITControlDialog* zKRAITControl = new ZKRAITControlDialog(this);
    zKRAITControl->setModel(walletModel);
    zKRAITControl->exec();
}

void PrivacyDialog::setZKRAITControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKRAITSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKRAIT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Krait Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zKRAITpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zKRAIT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " KRAIT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zKRAIT</b>";
    ...
    // use mints from zKRAIT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKRAITControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZKRAITControlDialog::GetSelectedMints();
    }
    ...
    // Clear zKRAIT selector in case it was used
    ZKRAITControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zKRAIT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " KRAIT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zKRAIT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zKRAIT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zKRAIT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zKRAIT "));
    ui->labelzKRAITAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzKRAITSyncStatus->setVisible(fShow);
}
